...
trait GeneralTrait
{
    ...
    public function returnSuccessMessage($msg = "", $errNum = "3500")
    {
        return response()->json([
            'status' => true,
            'errNum' => "3500",
    ...
    function encryptDatass($data) {
        // Pad the input data using PKCS7 padding
        $blockSize = 16; // AES-256-CBC uses a block size of 16 bytes
        $padding = $blockSize - (strlen($data) % $blockSize);
        $data .= str_repeat(chr($padding), $padding);

        // Encrypt the padded data with AES-256-CBC encryption
        $encryptedData = Crypt::encryptString($data, '12345678911111111111111111111111');

        return $encryptedData;
    }
    function encryptData($data) {
        // Pad the key to 32 characters (256 bits)
        $key = str_pad('12345678911111111111111111111111', 16, '*');

        // Encrypt the data with AES-256-CBC encryption
        $encryptedData = Crypt::encryptString($data, $key);

        return $encryptedData;
    }
    function encryptResponse($data) {
        $key ='12345678911111111111111111111111'; // مفتاح التطبيق الموجود في ملف .env
        $encryptedData = Crypt::encryptString($data, $key); // تشفير البيانات باستخدام المفتاح
        return response($encryptedData)->header('Content-Type', 'application/octet-stream'); // إرجاع الرد المشفر
    }
    function sadasdencryptData($data ) {
        // Pad the input data using PKCS7 padding
        $blockSize = 16; // AES-256-CBC uses a block size of 16 bytes
        $padding = $blockSize - (strlen($data) % $blockSize);
        $data .= str_repeat(chr($padding), $padding);
    ...
    function encryptDataasdasdas($data ) {
        // Pad the input data using PKCS7 padding
        $blockSize = 16; // AES-256-CBC uses a block size of 16 bytes
        $padding = $blockSize - (strlen($data) % $blockSize);
        $data .= str_repeat(chr($padding), $padding);
    ...
    public function returnDataCache($key, $value, $msg = "")
    {
        return  ([
            'status' => true,
            'errNum' => "S000",
